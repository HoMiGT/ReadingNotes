...
$$
\begin{aligned}
\text{参数量} = (kernelSize_h \cdot kernelSize_w \cdot inChannels + 1) \cdot outChannels
\end{aligned}
$$
...
则参数个数：(3×3×1+1)×32=(9+1)×32=320 所以这一层共有 320 个可训练参数。

```Python
import numpy as np

TX = np.load("./dataset/mnist/x_train.npy")
TY = np.load("./dataset/mnist/y_train.npy")


# 把值除以255，归一化到[0,1]之间，训练会收敛更快
def convertX(X):
...
# 从im形式变换为col形式
# im: 图像数 x 通道数 x 高 x 宽
# fh: 过滤器的高
# fw: 过滤器的宽
# s: 步进
# p: 填充
# 返回值: 形式为 "个数为图像数的特征图的高和宽的大小 x 过滤器的大小" 的矩阵
...
# 最大池化
def max_pooling(X, fh, fw):
    # 计算卷积后的特征图的大小
    N, IC, IH, IW = X.shape
    OH, OW = output_size(IH, fh, fh), output_size(IW, fw, fw)
    # 为了更容易选择最大值，变更数据的形式
    X = im2col(X, fh, fw, fh).reshape(N * OH * OW * IC, fh * fw)
    # 计算最大值及其索引
    P = X.max(axis=1)
    PI = X.argmax(axis=1)
...
def forward(X0):
    ...
    A1 = relu(Z1)
    X1, PI1 = max_pooling(A1, fh=2, fw=2)
    # 卷积层2
    Z2 = convolve(X1, W2, b2, s=1, p=2)
    A2 = relu(Z2)
    X2, PI2 = max_pooling(A2, fh=2, fw=2)
    # 展开为1列
    N = X2.shape[0]
    X2 = X2.reshape(N, -1)
    # 全连接层
...
# 从 col 形式变换成 im 的形式
# col: col形式的数据
# im_shape: 指定恢复为im形式时的 "图像数 x 通道数 x 高 x 宽"的大小
# fh：过滤器的高
# fw：过滤器的宽
# s: 步进
# p: 填充
...
# 测试集
testX = np.load("./dataset/mnist/x_test.npy")
testY = np.load("./dataset/mnist/y_test.npy")

testX = convertX(testX)
testY = convertY(testY)
show_images(testX[:10])
...
x_train = x_train.astype(np.float32) / 512.0
...
for epoch in range(epochs):
    ...
    for i in tqdm(range(train_num)):  # 开始循环训练
        ...
        for b in range(batch_size):
            img = x_train[np.random.randint(x_train.shape[0])]  # 提取单福图片内容
            x = img
            y = img
            x_imgs_batch.append(x)
            y_batch.append(y)
        # 将批次数据转化为PyTorch对应的tensor格式并将其传入GPU中
        x_imgs_batch = torch.from_numpy(np.array(x_imgs_batch)).float().to(device)
        y_batch = torch.tensor(y_batch).float().to(device)
        pred = model(x_imgs_batch)  # 对模型进行正向计算
        loss = torch.nn.MSELoss(reduction='sum')(pred, y_batch)/batch_size  # 使用损失函数进行计算
        # 下面是固定格式，一般这样使用即可
        optimizer.zero_grad()  # 对结果进行优化计算
        loss.backward()   # 损失值的反向传播
        optimizer.step()   # 对参数进行更新
        train_loss += loss.item()   # 计算每个批次的损失值
    # 计算并打印损失值
    ...
    image = torch.reshape(image, [28,28])  # 修正模型输出结果
